...
namespace Utils
{
        ...
        std::string const & memberName)
    {
        if (!value.is<T>())
        {
            throw SLabException("Error parsing JSON: member \"" + memberName + "\" is not of the expected type");
        }
        ...
        std::string const & memberName)
    {
        ...
        {
            throw SLabException("Error parsing JSON: member \"" + memberName + "\" is not of type 'object'");
        }
        ...
        std::string const & memberName)
    {
        ...
        if (obj.end() == memberIt)
        {
            throw SLabException("Error parsing JSON: cannot find member \"" + memberName + "\"");
        }
        ...
        std::string const & memberName)
    {
        ...
        if (!memberIt->second.is<picojson::object>())
        {
            throw SLabException("Error parsing JSON: requested member \"" + memberName + "\" is not of type 'object'");
        }
        ...
        std::string const & memberName)
    {
        ...
        {
            throw SLabException("Error parsing JSON: requested member \"" + memberName + "\" is not of type 'array'");
        }
    ...
    inline rgbColor Hex2RgbColor(std::string str)
    {
        ...
        if (str.length() != 6)
            throw SLabException("Error: badly formed hex color value \"" + str + "\"");
    ...
    inline std::string LoadTextFile(std::filesystem::path const & filepath)
    {
        ...
        if (!file.is_open())
        {
            throw SLabException("Cannot open file \"" + filepath.string() + "\"");
        }
